  ...
  return (
    <>
      ...
      <header className="fixed top-4 left-1/2 -translate-x-1/2 z-[1000] w-[95%] max-w-5xl transition-all duration-500">
        <nav 
          className={`relative w-full flex items-center justify-between bg-white/10 backdrop-blur-xl rounded-full px-3 py-2 shadow-2xl border border-white/20 transition-all duration-500 ${
            scrolled ? 'bg-white/5' : ''
          }`}
          style={{
        ...
        >
          ...
          <Link 
            to={user ? "/dashboard" : "/"}
            className={`flex-shrink-0 flex items-center gap-2 hover:scale-105 transition-all duration-300 ${
          ...
          {user && (
            <div 
              className={`hidden md:flex items-center gap-2 ml-auto mr-3 transition-all duration-500 ${
